...
import com.itechart.accidents.constants.GeneralConstants.{FIRST_STATION_ID, HASH_DIFFERENCE, LAST_STATION_ID}
import com.itechart.accidents.database.DataDatabase
import com.itechart.accidents.entity.{WeatherEntity, WeatherForAccident, WeatherStation}
import com.itechart.accidents.integration.ny.database.dao.WeatherDAO
import com.itechart.accidents.integration.ny.parse.NYWeatherParser
import com.itechart.accidents.spark.Spark
import com.itechart.accidents.spark.Spark.sparkSql.implicits._
import com.itechart.accidents.utils.{DateUtils, PostgisUtils}
...
@Singleton
class WeatherMappingService @Inject()(weatherDAO: WeatherDAO, weatherParser: NYWeatherParser) {

  private val allStations: Seq[WeatherStation] =
    Await.result(DataDatabase.database.run(weatherDAO.allStations()), Duration.Inf)
  ...
  private val allWeather: Map[Int, Map[Long, Seq[WeatherEntity]]] =
    Await.result(
      DataDatabase.database.run(weatherDAO.allWeather()), Duration.Inf)
