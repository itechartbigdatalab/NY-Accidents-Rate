...
class DateUtilsTest extends FunSpec {
  describe("DateUtils") {
    it("should return valid value") {
      ...
      println(DateUtils.parseDate(testValue, GeneralConstants.DATE_FORMATTER_WEATHER).isDefined)

      assert(DateUtils.parseDate(testValue, GeneralConstants.DATE_FORMATTER_WEATHER).contains(expectedResult))
    }
  }
}
